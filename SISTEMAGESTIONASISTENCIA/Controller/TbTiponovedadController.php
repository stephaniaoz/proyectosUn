...
include("C:\wamp64\www\SISTEMAGESTIONASISTENCIA\Model\TbTiponovedad.php");
...
* 
*/
class TbTiponovedadController
{

	private $arrayTipoNovedad = array();

	private $arrayUsuarioPerfil = array();
	private $arrayUsuarioCarrera =array();
	private $arrayListaMenu = array();

	function listarTiposNovedad(){

		$lista = "<option value='0'>Seleccione uno</option>";
		$objTiponovedad = new TbTiponovedad();
		$resultTipoNov = $objTiponovedad->listaTipoNovedad();
		...
		while ($arrayTipNov = pg_fetch_assoc($resultTipoNov)) {			
			...
			$lista .= "<option value='".$arrayTipNov['tipnov_codigo']."'>".$arrayTipNov['tipnov_nombre']."</option>";
			$count++;
		}
